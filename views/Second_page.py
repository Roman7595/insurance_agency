...
def count_contract_by_users_quary(window):
    sql_frame = LabelFrame(window.winfo_children()[-1], text='Количество договоров на человека ')
    sql_frame.grid(column=0,row=0,padx=20,)
...
def count_contract_where_payments_quary(window):
    sql_frame = LabelFrame(window.winfo_children()[-1], text='Договоры где есть выплата')
    sql_frame.grid(column=1,row=0,padx=20)
